...
class ListService {
    ...
    async editCardOrdering(id: string, body: PatchCardOrderingBody) { // there we will get drag and dropable card id
        validation.uuid_validation(id);

        const {orderingArr}: PatchCardOrderingBody = body;

        const drag_and_dropable_card: CardsModel | undefined = await getRepository(Card).findOne(id);

        if (!drag_and_dropable_card) {
            throw new NotFoundException()
        }
        ...
        if (orderingArr.length === 1) {
            ...
            // when card will be move in other list, we need to change cards list id column
            drag_and_dropable_card.list_id = orderingArr[1].list_id;
            drag_and_dropable_card.listId = orderingArr[1].list_id;

            await getRepository(Card).save(drag_and_dropable_card);
        }
